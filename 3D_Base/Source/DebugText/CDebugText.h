...
public:
	...
	struct SHADER_CONSTANT_BUFFER
	{
		...
		ALIGN16 float fViewPortWidth = 0;	//ビューポートの幅
		ALIGN16 float fViewPortHeight = 0;	//ビューポートの高さ
	};
	//頂点の構造体.
	struct VERTEX
	{
...
private:
	...
	ComPtr<ID3D11Buffer>		m_pConstantBuffer;	//コンスタントバッファ.

	ComPtr<ID3D11Buffer>		m_pVertexBuffer[SPRITE_MAX];	//頂点バッファ.

	ComPtr< ID3D11ShaderResourceView>	m_pTexture;			//テクスチャ.
