	...
	for (int x = 0; x < SPRITE_MAX_W; x++)
	{
		for (int y = 0; y < SPRITE_MAX_H; y++)
		{
			...
			if (FAILED(m_pDevice11->CreateBuffer(
				&bd, &InitData, m_pVertexBuffer[count].GetAddressOf())))
			{
...
void CDebugText::RenderFont(int FontIndex, float x, float y)
{
	...
	m_pContext11->IASetVertexBuffers( 0, 1,
		m_pVertexBuffer[FontIndex].GetAddressOf(), &stride, &offset);
...
void CDebugText::Render(LPCTSTR text, int x, int y)
{
	...
	ID3D11Buffer* pBuffers[1] = { m_pConstantBuffer.Get() };
	m_pContext11->VSSetConstantBuffers( 0, 1, pBuffers);
	m_pContext11->PSSetConstantBuffers( 0, 1, pBuffers);
