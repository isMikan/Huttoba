#include "CPlayerResultWin_TypeC.h"
...
CPlayerResultWin_TypeC::CPlayerResultWin_TypeC(CPlayerBase& pPlayer)
	...
	, m_RightHandPos		()
	, m_LeftHandPos			()
	, m_RightHandEndPos		( 0.f, 0.f, -0.2f )
	, m_LeftHandEndPos		( -0.2f, 0.3f, 0.4f )

{
}

CPlayerResultWin_TypeC::~CPlayerResultWin_TypeC()
{
}
...
//--- 状態の開始時に呼び出す ---.
void CPlayerResultWin_TypeC::Enter()
{
	//攻撃の開始時間を取得.
...
//--- 状態の終了時に呼び出す ---.
void CPlayerResultWin_TypeC::Exit()
{
}

//--- この状態の間に呼び出す ---.
void CPlayerResultWin_TypeC::Update()
{
	//経過時間を取得.
