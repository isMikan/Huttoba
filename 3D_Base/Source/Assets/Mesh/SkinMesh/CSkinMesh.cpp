	...
	CDirectX9& pDx9, CDirectX11& pDx11, LPCTSTR FileName )
{
	...
	//コンスタントバッファ(メッシュごと).
	if( FAILED(
		CreateCBuffer( m_pCBufferPerMesh.GetAddressOf(), sizeof(CBUFFER_PER_MESH))))
	{
		return E_FAIL;
	}
	//コンスタントバッファ(メッシュごと).
	if( FAILED(
		CreateCBuffer( m_pCBufferPerMaterial.GetAddressOf(), sizeof(CBUFFER_PER_MATERIAL))))
	{
		return E_FAIL;
	}
	//コンスタントバッファ(メッシュごと).
	if( FAILED(
		CreateCBuffer( m_pCBufferPerFrame.GetAddressOf(), sizeof(CBUFFER_PER_FRAME))))
	{
		return E_FAIL;
	}
	//コンスタントバッファ(メッシュごと).
	if( FAILED(
		CreateCBuffer( m_pCBufferPerBone.GetAddressOf(), sizeof(CBUFFER_PER_BONES))))
	{
		return E_FAIL;
	}
	//テクスチャー用サンプラー作成.
	if( FAILED( CreateSampler( m_pSampleLinear.GetAddressOf() ) ) )
	{
		return E_FAIL;
	}
...
HRESULT	CSkinMesh::CreateShader()
{
	...
	if( FAILED(
		m_pDevice11->CreateInputLayout(
			...
			pCompiledShader->GetBufferSize(), m_pVertexLayout.GetAddressOf() ) ) )
	{
...
HRESULT CSkinMesh::LoadXMesh( LPCTSTR FileName )
{
	...
	//Xファイル読み込み.
	m_pD3dxMesh = std::make_unique<D3DXPARSER>();
	...
	for( DWORD i = 0; i < pAppMesh->NumMaterial; i++ )
	{
		...
		if( pAppMesh->pMaterial[i].TextureName[0] != 0
			&& FAILED(
				...
				m_pDevice11.Get(), pAppMesh->pMaterial[i].TextureName,
...
void CSkinMesh::SendCBufferPerBone(	SKIN_PARTS_MESH* pMesh )
{
			...
			D3D11_MAP_WRITE_DISCARD, 0, &pData ) ) )
	{
		...
		m_pContext11->Unmap( m_pCBufferPerBone.Get(), 0);
	}
	m_pContext11->VSSetConstantBuffers( enCBSlot::Bones, 1, m_pCBufferPerBone.GetAddressOf() );
	m_pContext11->PSSetConstantBuffers(enCBSlot::Bones, 1, m_pCBufferPerBone.GetAddressOf());
}
...
void CSkinMesh::SendCBufferPerFrame()
{
	...
	m_pContext11->VSSetConstantBuffers( enCBSlot::Frame, 1, m_pCBufferPerFrame.GetAddressOf());
	m_pContext11->PSSetConstantBuffers(enCBSlot::Frame, 1,  m_pCBufferPerFrame.GetAddressOf());
}
...
void CSkinMesh::SendCBufferPerMesh()
{
	...
	m_pContext11->VSSetConstantBuffers( enCBSlot::Mesh, 1, m_pCBufferPerMesh.GetAddressOf() );
	m_pContext11->PSSetConstantBuffers( enCBSlot::Mesh, 1, m_pCBufferPerMesh.GetAddressOf() );
}
...
void CSkinMesh::SendCBufferPerMaterial( MY_SKINMATERIAL* pMaterial )
{
	...
	m_pContext11->VSSetConstantBuffers( enCBSlot::Material, 1, m_pCBufferPerMaterial.GetAddressOf() );
	m_pContext11->PSSetConstantBuffers( enCBSlot::Material, 1, m_pCBufferPerMaterial.GetAddressOf() );
}
...
void CSkinMesh::SendTexture( MY_SKINMATERIAL* pMaterial )
{
	...
	{
		m_pContext11->PSSetSamplers( 0, 1, m_pSampleLinear.GetAddressOf() );
...
HRESULT CSkinMesh::CreateSampler( ID3D11SamplerState** pSampler )
{
	...
	if (FAILED(
		m_pDevice11->CreateSamplerState( &SamDesc, m_pSampleLinear.GetAddressOf() )))
	{
		return E_FAIL;
	}
	return S_OK;
