...
HRESULT CStaticMesh::CreateMaterials()
{
	...
	//マテリアル数分の領域を確保.
	m_pMaterials.resize(m_Model.NumMaterials);
	//マテリアル数分繰り返し.
	for( DWORD No = 0; No < m_Model.NumMaterials; No++ )
	{
			...
			&& strlen( d3dxMaterials[No].pTextureFilename ) > 0 )
		{
			...
			if( FAILED( D3DX11CreateShaderResourceViewFromFile(
				m_pDevice11.Get(), m_pMaterials[No].TextureName,//テクスチャファイル名.
				nullptr, nullptr,
...
HRESULT CStaticMesh::CreateIndexBuffer()
{
	...
	for( DWORD No = 0; No < m_NumAttr; No++ )
	{
		...
		if( FAILED( m_pDevice11->CreateBuffer(
			&bd, &InitData, m_ppIndexBuffer[No].GetAddressOf() ) ) )
		{
...
HRESULT CStaticMesh::CreateVertexBuffer()
{
		...
		pVB->Lock( 0, 0, (VOID**)&pVertices, 0 ) ) )
	{
		...
		if( FAILED( m_pDevice11->CreateBuffer(
			&bd, &InitData, m_pVertexBuffer.GetAddressOf()) ) )
		{
...
HRESULT CStaticMesh::CreateSampler()
{
	...
	if( FAILED( m_pDevice11->CreateSamplerState(
		&samDesc, m_pSampleLinear.GetAddressOf() ) ) )//(out)サンプラ.
	{
...
HRESULT CStaticMesh::CreateShader()
{
	...
	if( FAILED(
		m_pDevice11->CreateVertexShader(
			...
			nullptr,
			m_pVertexShader.GetAddressOf() )))	//(out)バーテックスシェーダ.
	{
	...
	if (FAILED(
		m_pDevice11->CreateInputLayout(
			...
			pCompiledShader->GetBufferPointer(),
			pCompiledShader->GetBufferSize(),
			m_pVertexLayout.GetAddressOf())))	//(out)頂点インプットレイアウト.
	{
	...
	if( FAILED(
		m_pDevice11->CreatePixelShader(
			...
			pCompiledShader->GetBufferSize(),
			nullptr,
			m_pPixelShader.GetAddressOf() )))	//(out)ピクセルシェーダ.
	{
...
HRESULT CStaticMesh::CreateConstantBuffer()
{
	...
	//コンスタントバッファの作成.
	if( FAILED(
		m_pDevice11->CreateBuffer( &cb, nullptr, m_pCBufferPerMesh.GetAddressOf() ) ) )
	{
	...
	//コンスタントバッファの作成.
	if( FAILED(
		m_pDevice11->CreateBuffer( &cb, nullptr, m_pCBufferPerMaterial.GetAddressOf() ) ) )
	{
	...
	//コンスタントバッファの作成.
	if( FAILED(
		m_pDevice11->CreateBuffer( &cb, nullptr, m_pCBufferPerFrame.GetAddressOf() ) ) )
	{
	...
	LIGHT& Light, D3DXVECTOR3& CamPos)
{
	...
	m_pContext11->PSSetShader( m_pPixelShader.Get(), nullptr, 0);	//ピクセルシェーダ.
	...
	D3DXMATRIX& mWorld, D3DXMATRIX& mView, D3DXMATRIX& mProj)
{
	...
	ID3D11Buffer* pCBufferPerMesh[1] = { m_pCBufferPerMesh.Get() };
	//このコンスタントバッファをどのシェーダで使用するか？.
	m_pContext11->VSSetConstantBuffers( 0, 1, pCBufferPerMesh);	//頂点シェーダ.
	m_pContext11->PSSetConstantBuffers( 0, 1, pCBufferPerMesh );	//ピクセルシェーダ.
