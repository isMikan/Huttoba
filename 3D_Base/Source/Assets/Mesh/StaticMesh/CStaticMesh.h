...
public:
	...
	void SetObjectColor(
		...
		const D3DXVECTOR4& specular)
	{
		CBUFFER_PER_MATERIAL color = { diffuse, ambient, specular };
...
private:
	...
	ComPtr<ID3D11PixelShader>		m_pPixelShader;		//ピクセルシェーダ.

	ComPtr<ID3D11Buffer>			m_pCBufferPerMesh;		//コンスタントバッファ(メッシュ毎).
	ComPtr<ID3D11Buffer>			m_pCBufferPerMaterial;	//コンスタントバッファ(マテリアル毎).
	ComPtr<ID3D11Buffer>			m_pCBufferPerFrame;		//コンスタントバッファ(フレーム毎).

	ComPtr<ID3D11Buffer>				m_pVertexBuffer;	//頂点バッファ.
	std::vector<ComPtr<ID3D11Buffer> >	m_ppIndexBuffer;	//インデックスバッファ.
	ComPtr<ID3D11SamplerState>			m_pSampleLinear;	//サンプラ:テクスチャに各種フィルタをかける.

	MY_MODEL		m_Model;
	MY_MODEL		m_ModelForRay;	//レイとの当たり判定で使用する.

	std::vector<MY_MATERIAL>	m_pMaterials;	//マテリアル構造体.
	DWORD			m_NumAttr;		//属性数.
	...
	std::unordered_map<int, CBUFFER_PER_MATERIAL>	m_ObjectColor;		//オブジェクトの色.
