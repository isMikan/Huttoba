...
//初期化(DirectX固定)
HRESULT CSprite2D::Init(LPCTSTR lpFileName, SPRITE_STATE& pSs)
{
...
HRESULT CSprite2D::CreateTexture( LPCTSTR lpFileName )
{
	...
	if( FAILED( D3DX11CreateShaderResourceViewFromFile(
		m_pDevice11.Get(),		//リソースを使用するデバイスのポインタ.
		lpFileName,			//ファイル名.
		nullptr, nullptr,
		m_pTexture.GetAddressOf(),		//(out)テクスチャ.
		nullptr ) ) )
	{
...
void CSprite2D::Render()
{
	...
	if (SUCCEEDED(
		m_pContext11->Map( m_pConstantBuffer.Get(),
			0, D3D11_MAP_WRITE_DISCARD, 0, &pData )))
	{
		...
		m_pContext11->Unmap(m_pConstantBuffer.Get(), 0);
	}

	//このコンスタントバッファをどのシェーダで使うか？.
	ID3D11Buffer* pCBufferPerFrame[1] = { m_pConstantBuffer.Get()};
	m_pContext11->VSSetConstantBuffers( 0, 1, pCBufferPerFrame);
	m_pContext11->PSSetConstantBuffers( 0, 1, pCBufferPerFrame);
	...
	m_pContext11->PSSetShaderResources( 0, 1, m_pTexture.GetAddressOf() );
