...
HRESULT CDirectX11::Create(HWND hWnd)
{
	...
	m_pContext11->OMSetRenderTargets(
		1,
		m_pBackBuffer_TexRTV.GetAddressOf(),
		m_pBackBuffer_DSTexDSV.Get() );
			...
			m_pContext11.GetAddressOf() ) ) )			//(out)デバイス　コンテキスト.
	{
		...
		if( FAILED(
			D3D11CreateDeviceAndSwapChain(
				...
				&sd, m_pSwapChain.GetAddressOf(), m_pDevice11.GetAddressOf(),
				pFeatureLevel, m_pContext11.GetAddressOf() ) ) )
		{
			//リファレンスデバイスの作成.
...
HRESULT CDirectX11::CreateDepthStencilState()
{
	...
	if( FAILED(
		m_pDevice11->CreateDepthStencilState(
			&dsDesc, m_pDepthStencilStateOn.GetAddressOf() ) ) )
	{
	...
	if( FAILED(
		m_pDevice11->CreateDepthStencilState(
			&dsDesc, m_pDepthStencilStateOff.GetAddressOf() ) ) )
	{
...
HRESULT CDirectX11::CreateAlphaBlendState()
{
	...
	//ブレンドステート作成.
	if( FAILED(
		m_pDevice11->CreateBlendState( &BlendDesc, m_pAlphaBlendOn.GetAddressOf() ) ) )
	{
...
void CDirectX11::SetAlphaBlend( bool flag )
{
	...
	ID3D11BlendState* pTmp
		= ( flag == true ) ? m_pAlphaBlendOn.Get() : m_pAlphaBlendOff.Get();
...
void CDirectX11::SetDepth(bool flag)
{
	ID3D11DepthStencilState* pTmp
		= ( flag == true ) ? m_pDepthStencilStateOn.Get() : m_pDepthStencilStateOff.Get();
...
{
	//画面のクリア.
	//float ClearColor[4] = { 1.0f, 0.85f, 0.7f, 1.0f };	//クリア色（RGBAの順）.
	float ClearColor[4] = { 0.9f, 0.9f, 0.9f, .2f };	//クリア色（RGBAの順）.
	//カラーバックバッファ.
	m_pContext11->ClearRenderTargetView(
		m_pBackBuffer_TexRTV.Get(), ClearColor);
	//デプスステンシルバックバッファ.
...
HRESULT CDirectX11::CreateColorBackBufferRTV()
{
	...
	if( FAILED(
		m_pDevice11->CreateRenderTargetView(
			pBackBuffer_Tex,
			nullptr,
			m_pBackBuffer_TexRTV.GetAddressOf() ) ) )	//(out)RTV.
	{
...
HRESULT CDirectX11::CreateDepthStencilBackBufferRTV()
{
	...
	if( FAILED(
		m_pDevice11->CreateTexture2D(
			&descDepth,
			nullptr,
			m_pBackBuffer_DSTex.GetAddressOf() ) ) )	//(out)デプスステンシル用テクスチャ.
	{
		return E_FAIL;
	}
	...
	if( FAILED( m_pDevice11->CreateDepthStencilView(
		m_pBackBuffer_DSTex.Get(),
		nullptr,
		m_pBackBuffer_DSTexDSV.GetAddressOf() ) ) )	//(out)DSV.
	{
